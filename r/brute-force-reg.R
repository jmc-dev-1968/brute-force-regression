...
TestIt <- function(kmid1, kmid2, timeid, model, trimper=NULL) 
{
  ...
  # get var names
  tsql = sprintf("select LongName, 1 AS OrdID from DEV.un.v_KPI where KMID = %s UNION 
                 select LongName, 2 AS OrdID from DEV.un.v_KPI where KMID = %s ORDER BY 2", kmid1, kmid2)
...
for(i in 1:length(loop[,1]))
{
  ...
  for(j in 1:length(loop[,1]))
  {
    ...
    if(m_pathid1 != m_pathid2 && m_kmid1 < m_kmid2) 
      ...
      if(rowsEqual)
      {
      	isql = sprintf("INSERT INTO DEV.un.Reg(x_kmid, y_kmid, timeid, n, model, error_desc) VALUES(%d, %d, %d, %d,'%s','%s')",
      	               m_kmid1, m_kmid2, timeid, n, model, "EQUAL VALUES IN A COLUMN")
      ...
      {
        isql = sprintf("INSERT INTO DEV.un.Reg(x_kmid, y_kmid, timeid, n, model, error_desc) VALUES(%d, %d, %d, %d,'%s','%s')",
                       m_kmid1, m_kmid2, timeid, n, model, "INSUFFICIENT DATA")
        sqlQuery(conn, isql, errors = TRUE)
      }
